...
namespace qx {
    ...
    public:
        ...
        binary_counter(uint64_t bit_count)
        {
            assert(bit_count <= 64);
            __num = bit_count;
            __max = static_cast<uint64_t>(1 << bit_count);
            __val = 0;
        }

        void set(uint32_t b)
        {
            __val |= static_cast<uint64_t>(1 << b);
        }
        ...
        {
            return (__val & static_cast<uint64_t>(1 << b));
        }
        ...
        {
            print("[ ");
            for (uint64_t i = __num; i > 0; i--)
                print((test(static_cast<uint32_t>(i - 1)) ? '1' : '0'));
            println(" : " << __val << "]");
        }
